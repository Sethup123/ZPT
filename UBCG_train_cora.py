...
DEVICE = 'cuda'
DATA_DIR = f'./G2P2-conditional-main/save_folder/{opt.data_name}_node_text_emb_neig.zip'
# ============================================================ #
# UBCG Model:
...
def UBCG_Loss(x, x_recon, mean, log_var):
...
train_feat = normalize(matcontent['node_emb'])
train_cond = normalize(np.array(attrs))
core_text = normalize(core_text)
...
for epoch in range(n_epoch):
    ...
    for (x, cond, core_text) in train_dataloader:
        ...
        recon_node, kl_node, loss_node = UBCG_Loss(x, x_recon_node, mean_node, log_var_node)

        x_recon_text, sampling_text, mean_text, log_var_text = model(cond, x)
        recon_text, kl_text, loss_text = UBCG_Loss(cond, x_recon_text, mean_text, log_var_text)
        ...
        loss.backward()
        optim.step()
        # print(x_recon[0], sampling[0], mean[0], log_var[0])
        # break
