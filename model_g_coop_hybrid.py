...
class CoOp_synthetic(nn.Module):
    """Context Optimization (CoOp).
    Learning to Prompt for Vision-Language Models
    https://arxiv.org/abs/2109.01134
    """
    ...
    def __init__(self, args, classnames, clip_model, g_texts, device):
        ...
        self.model = PromptLearner(args, classnames, clip_model, g_texts)
        self.tokenized_prompts = self.model.tokenized_prompts
        ...
        self.optim = optim.Adam(self.model.parameters(), lr=args.prompt_lr)

    def forward(self, node_feat, text_feat, label, training=True):
        prompts = self.model()
        tokenized_prompts = self.tokenized_prompts
        prompt_features = self.text_encoder(prompts, tokenized_prompts)
        
        image_features = node_feat / node_feat.norm(dim=-1, keepdim=True)
        text_features = text_feat / text_feat.norm(dim=-1, keepdim=True)
        
        prompt_features = prompt_features / prompt_features.norm(dim=-1, keepdim=True)

        logit_scale = self.logit_scale.exp()
        logits1 = logit_scale * image_features @ prompt_features.t()
        logits2 = logit_scale * text_features @ prompt_features.t()
        
        
        logits = self.args.lamda * logits1 + (1-self.args.lamda) * logits2
